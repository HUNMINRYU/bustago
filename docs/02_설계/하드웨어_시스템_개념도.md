...
> **작성일:** 2026-04-29  
> **버전:** v1.0  
> **작성 배경:** 4/30 교수 피드백 — 하드웨어 아키텍처 구조도·시스템 개념도 부재 지적 및 Pi→Jetson 변경 근거 누락 지적 반영
...
graph TB
    subgraph STOP["🚏 인성관 셔틀 정류장"]
        ...
        JETSON["⚡ Jetson Orin Nano 8GB<br/>YOLOv8-nano + DeepSORT<br/>Line Crossing Counter<br/>25~40 FPS"]
...
Pi 4는 CPU 기반으로 YOLOv8 추론 시 **2~5 FPS**에 불과하다.  
...
│         (AI 전용)           │     │       (키오스크 전용)        │
├─────────────────────────────┤     ├─────────────────────────────┤
│ • YOLOv8-nano 추론 (GPU)    │     │ • Chromium --kiosk 모드     │
...
Pi Camera v2 (30 FPS 캡처)
    → Jetson: YOLOv8-nano 탐지 (25~40 FPS)
...
| 레이어 | 보드 | 기술 |
|--------|------|------|
| AI 추론 | Jetson Orin Nano | JetPack 6.x, YOLOv8-nano TensorRT FP16, DeepSORT, OpenCV(CUDA) |
...
| ML 모델 | 서버 | RandomForest (n=10, depth=10), 7 feature, CV 0.9962, 0.2MB |
...
- [YOLOv8 Jetson Benchmarks](https://docs.ultralytics.com/guides/nvidia-jetson/) — YOLOv8n TensorRT FP16 ≈ 40 FPS on Jetson Orin Nano
